...
blogsRouter.put('/:id', async (request, response) => {
  ...
  const blogModif = await Blog.findByIdAndUpdate(request.params.id, blogToUp, {
    new: true,
    runValidators: true,
  })
  if (blogModif) {
    const result = await blogModif.populate('user', {
      username: 1,
      name: 1,
      id: 1,
    })
    ...
    response
      .status(400)
      .json({ error: `${blogToUp.title} was already removed from server` })
  }
})
